...
class EMT_Lib
{
	...
	*/
	...
	{
		if ($way)
			$text = preg_replace_callback('/(\<\/?)(.+?)(\>)/s', function($m) { return $m[1].( substr(trim($m[2]), 0, 1) === "a" ? "%%___"  : ""  ) . EMT_Lib::encrypt_tag(trim($m[2]))  . $m[3];}, $text);
		else
			$text = preg_replace_callback('/(\<\/?)(.+?)(\>)/s', function($m) { return $m[1].( substr(trim($m[2]), 0, 3) === "%%___" ? EMT_Lib::decrypt_tag(substr(trim($m[2]), 4)) : EMT_Lib::decrypt_tag(trim($m[2])) ) . $m[3];}, $text);
		return $text;
	}
